...
argToType
  ...
  -> Typing
  -> Infer Type
argToType _ gens typeDecls _ params (Canonical _ (TRSingle n))
...
argToType priorEnv gens typeDecls name params (Canonical _ (TRComp tname targs))
...
argToType priorEnv gens typeDecls name params (Canonical _ (TRArr l r)) = do
  l' <- argToType priorEnv gens typeDecls name params l
  r' <- argToType priorEnv gens typeDecls name params r
  return $ l' `fn` r'

argToType priorEnv gens typeDecls name params (Canonical _ (TRRecord f)) = do
  f' <- mapM (argToType priorEnv gens typeDecls name params) f
  return $ TRecord f' False

argToType priorEnv gens typeDecls name params (Canonical _ (TRTuple elems)) =
  do
