...
buildPipeAbs area exps = Src.Source emptyInfos area (Src.Abs "__x__" $ [buildPipeAbs' (Src.Source emptyInfos area (Src.Var "__x__")) exps])

buildPipeAbs' :: Src.Exp -> [Src.Exp] -> Src.Exp
buildPipeAbs' prev [] = prev
buildPipeAbs' prev (abs:exps) =
  let application = Src.Source emptyInfos (mergeAreas (Src.getArea prev) (Src.getArea abs)) (Src.App ((Src.Source emptyInfos (Src.getArea abs)
          (Src.App (Src.Source emptyInfos (Src.getArea abs) (Src.Var "|>")) ( prev )  False)
        )) abs True)
  in
