...
resolveAbsoluteSrcPath pathUtils rootPath path = case getPathType path of
  FileSystemPath -> Just <$> canonicalizePath pathUtils
...
retrieveMadlibDotJson pathUtils dir = do
  ...
  if found
    then do
      ...
      then return Nothing
      else retrieveMadlibDotJson pathUtils $ getParentFolder dir
...
findMadlibPackageMainPath pathUtils file = do
  ...
  case json of
    Left _ -> return Nothing
    Right json' ->
      Just <$> canonicalizePath pathUtils (joinPath [folder, MadlibDotJSON.main json'])
